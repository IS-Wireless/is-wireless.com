<template>
  <section class="relative w-full mb-8 tablet-wide:mb-14 pt-10 tablet-wide:pt-16" :class="{' bg-black/50 tablet-wide:bg-black/5 overflow-hidden':backgroundUrl}" >
    ...
    <div class="absolute -z-10 bottom-0 bg-[25%_center] mx-auto desktop:bg-cover transform w-full min-h-screen h-[120%]" ref="parallax" v-if="backgroundUrl" v-bind:style="{ 'background-image': 'url(' + backgroundUrl + ')'}"></div>
...
export default {
    ...
    methods: {
      handleParallax() {
        ...
        element.style.transform = 'translateY(' +  (((window.top.scrollY - (this.$el.clientTop + this.$el.clientHeight) )/100)) +'%)';
      },
    },
    mounted() {
