<template>
    <nav class="w-full relative bg-black border-0 border-b border-gray-main">
        <div class="mx-auto w-4/5 flex justify-between">
            ...
            <ul class="w-full flex items-center justify-end flex-wrap">
                <div class=" h-full flex-shrink-0 flex items-center justify-end">
                    <li class="navbar-item flex group" v-for="items in menu" v-bind:key="items.title">
                        <nuxt-link class="text-base text-white group-hover:text-blue-main transition" to="">{{items.title}}</nuxt-link> 
                    </li>
                </div>
                ...
                <li class="navbar-item hidden tablet-wide:flex">
        ...
        <div class="fixed flex z-40 top-0 left-0 w-full h-screen overflow-hidden" v-bind:class="{'pointer-events-none':!expanded}">
            <div class="w-full transition duration-500 " v-bind:class="{'bg-[#3F3F3F]/70':expanded}" @click="toggleExpanded"></div>
            <div class="absolute z-50 top-0 left-full w-full tablet-wide:w-[300px] h-screen bg-[#242424] transform transition duration-500" v-bind:class="{'-translate-x-full ':expanded}">
                ...
                <ul class="flex flex-col-reverse px-7 mt-10">
                    <li class="mb-3 tablet:hidden" v-for="items in menu" v-bind:key="items.title">
