<template>
    <section class="relative z-10 w-full bg-gray-light">
      ...
      >
        ...
        <CustomLink :url="data.link" class="block mt-5 tablet:mt-0 w-full flex-grow-0 h-[50px] tablet:h-[60px] overflow-hidden">
            
                    <div
                      id="pressSwiper"
                      class="swiper swiper-container overflow-hidden"
                    >
                      ...
                      <div class="absolute h-full w-10 right-0 top-0 bg-gradient-to-l from-gray-light z-20">
                      </div>
                      <div class="absolute h-full w-10 left-0 top-0 bg-gradient-to-r from-gray-light z-20">
                      </div>
                    </div>
      ...
      logos() {
        ...
        if (downloadedData) {
            downloadedData.forEach((item) => {
                if (item.image?.url && !logosSwiper.some(e => e.url == item.image.url)) {
