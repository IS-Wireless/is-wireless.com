<template>
  <div class="w-full">
    ...
    v-if="data.use_filters"
      ...
      :class="{
        'shadow-[0_15px_10px_-15px_rgb(0_0_0_/_0.1),_0_4px_6px_-6px_rgb(0_0_0_/_0.1)]':
          filterBar && barPosition.top == 0,
      }"
    >
      <div
...
export default {
  ...
  setup() {
    ...
    const barPosition = reactive(useElementBounding(filterBar))
  ...
  methods: {
    getData() {
      if (this.$props.data.use_courses) {
        return useGeneralStore().getOptionsData.collection
      }
      if (this.$props.data.use_materials) {
        return useGeneralStore().getOptionsData.materials
      }
